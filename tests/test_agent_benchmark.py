...
def run_collector(tmp_path: Path, *extra: str) -> dict:
    ...
    subprocess.run([str(SCRIPT), "--repo", "/repo/dj-digger", "--codex-home", str(home), "--limit", "2", "--output", str(output), *extra], check=True)
...
def test_cumulative_usage_uses_last_snapshot_and_unknown_events_are_counted(tmp_path):
    ...
    legacy = next(s for s in report["sessions"] if s["thread_count"] == 1 and s["tokens"]["total"] == 28)
...
def test_schema_and_csv_output(tmp_path):
    ...
    output = tmp_path / "report.json"; csv_path = tmp_path / "report.csv"
    subprocess.run([str(SCRIPT), "--repo", "/repo/dj-digger", "--codex-home", str(home), "--limit", "1", "--output", str(output), "--csv", str(csv_path)], check=True)
...
def test_rejects_source_output(tmp_path):
    home = tmp_path / "codex"; (home / "sessions").mkdir(parents=True)
    result = subprocess.run([str(SCRIPT), "--repo", str(ROOT), "--codex-home", str(home), "--limit", "1", "--output", str(ROOT / "README.generated.json")], capture_output=True, text=True)
