"""Rich rendering adapter for presentation-neutral core progress events."""
...
class RichProgressReporter:
    """Render core progress events in one transient live terminal area."""
    ...
    def handle(self, event: ProgressEvent) -> None:
        if event.kind == "phase_started":
            self.phase_started(event.subject or "", event.completed or 0, event.total or 0)
        elif event.kind == "phase_finished":
            self.phase_finished(event.subject or "", event.completed or 0, event.total or 0)
        elif event.kind == "analysis_started":
            self.analysis_started(total=event.total or 0, completed=event.completed or 0)
        elif event.kind == "analysis_advanced":
            self.analysis_advanced()
        elif event.kind == "analysis_finished":
            self.analysis_finished()
        elif event.kind == "diagnostic" and event.subject is not None:
            level, _, message = event.subject.partition(":")
