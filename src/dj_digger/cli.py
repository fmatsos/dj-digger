...
def _review_sort_key(group: Any) -> tuple[int, float, float, str]:
    ...
    for member in group.members:
        ...
        if comparison is None:
            continue
        for name in (
            "active_loudness_delta_db", "true_peak_delta_db",
            "plr_delta_db", "gain_deficit_delta_db",
        ):
            value = getattr(comparison, name)
            if value is not None:
                deltas.append(abs(value))
    return (0 if deficits else 1, -(max(deficits) if deficits else 0.0),
            -(max(deltas) if deltas else 0.0), group.group_id)
